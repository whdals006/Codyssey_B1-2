...
처음 미션을 수행할 때는 OOM, CPU Spike, Deadlock의 개념을 정확히 모르는 상태였기에, 환경변수를 하나씩 바꾸어 가며 실행 로그를 보고 장애 원인을 파악했습니다.

만약 다시 미션을 수행한다면 무작정 실행하기보다, '환경변수를 이렇게 수정하면 이러한 장애가 발생할 것이다'라는 가설을 먼저 세운 뒤, 로그와 관제 데이터를 통해 이를 검증하는 방식으로 접근하겠습니다.

현상이 발생한 후 사후에 원인을 찾는 방식은 많은 시간과 비용이 낭비되지만, 가설 검증 기반의 디버깅은 장애 원인의 범위를 빠르게 좁혀 복구 시간을 획기적으로 단축할 수 있기 때문입니다.
